...
class RecipeObject {
    ...
    func addRecipe(ttl: String, yld: Int, pHour: Int, pMin: Int, cHour: Int, cMin: Int, rHour: Int, rMin: Int, ingred: String, direct: String, image: UIImage){
        
        var newRecipe = Recipe(title: ttl, yield: yld, prepHour: pHour, prepMin: pMin, cookHour: cHour, cookMin: cMin, readyHour: rHour, readyMin: rMin, ingredients: ingred, directions: direct, image: image)
        
        recipes.append(newRecipe)
    }
    
    func editRecipe(selection: Int, ttl: String, yld: Int, pHour: Int, pMin: Int, cHour: Int, cMin: Int, rHour: Int, rMin: Int, editImage: UIImage){
        ...
        recipes[selection].image = editImage
    }
    ...
    func loadRecipes() {
        ...
        query.getObjectInBackgroundWithId("0h4bmjUfZv") {
            ...
            if error == nil {
                ...
                var blankImage = UIImage(named: "Blank.png")
                
                for var i = 0; i < pDirections.count; i++ {
                    recipeBook.addRecipe(pTitle[i], yld: pYield[i], pHour: pPrepHour[i], pMin: pPrepMin[i], cHour: pCookHour[i], cMin: pCookMin[i], rHour: pReadyHour[i], rMin: pReadyMin[i], ingred: pIngredients[i], direct: pDirections[i], image: blankImage!)
                }
